...
export function renovarCredito(InfoCredito) {
  let disabled;
  let fechaVencimiento = new Date(InfoCredito?.fechaDesembolsado);
  fechaVencimiento.setMinutes(
    fechaVencimiento.getMinutes() + fechaVencimiento.getTimezoneOffset()
  );
  let result = addDays(new Date(fechaVencimiento), 30);
  result.setMinutes(result.getMinutes() + result.getTimezoneOffset());
  let renovacion = differenceInCalendarDays(result, hoy);

  if (InfoCredito?.fechaDesembolsado === null) {
    disabled = true;
